...
function base64ToBytes2(base64) {
  const len2 = base64.replace(/=+$/, "").length;
  const n = Math.floor(len2 * 3 / 4);
  const out = new Uint8Array(n);
  let i2 = 0;
  let j = 0;
  while (i2 < base64.length) {
    const a = BASE64_ALPHABET.indexOf(base64[i2++] ?? "");
    const b = BASE64_ALPHABET.indexOf(base64[i2++] ?? "");
    const c = BASE64_ALPHABET.indexOf(base64[i2++] ?? "");
    const d = BASE64_ALPHABET.indexOf(base64[i2++] ?? "");
    if (a < 0 || b < 0)
      break;
    out[j++] = a << 2 | b >> 4;
    if (c >= 0 && j < n)
...
var fetchWithEncryptedResponse = (sendRequester, config) => {
...
var DECRYPT_URL = "https://www.ciphertools.org/tools/aes/gcm";
var DECRYPT_STEPS = [
...
var onCronTrigger = (runtime2) => {
  ...
  const result = confHTTPClient.sendRequest(runtime2, fetchWithEncryptedResponse, consensusIdenticalAggregation())(runtime2.config).result();
  const bodyBytes = base64ToBytes2(result.bodyBase64);
  ...
  runtime2.log("--- Copy-paste for CipherTools (https://www.ciphertools.org/tools/aes/gcm) ---");
  runtime2.log("Ciphertext + tag (paste into Ciphertext + tag input, hex):");
  runtime2.log(ciphertextAndTagHex);
  runtime2.log("Nonce/IV (paste into Nonce / IV field, hex):");
  runtime2.log(nonceHex);
  runtime2.log("---");
  runtime2.log(DECRYPT_STEPS);
